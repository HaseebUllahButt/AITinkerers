...
function LoginContent({ devEnabled, googleEnabled }: Flags) {
  const params = useSearchParams();
  const error = params.get("error");
  const email = params.get("email");

  const [devEmail, setDevEmail] = useState("dev@localhost");
  const hasGoogle = googleEnabled;
  const hasDev = devEnabled;
...
export type Flags = { devEnabled: boolean; googleEnabled: boolean };

export default function LoginForm(flags: Flags) {
  return (
