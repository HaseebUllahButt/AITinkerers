...
export const HERMES_TOOLS: Anthropic.Tool[] = [
  ...
  T({ name: "prospect_detail", description: "Everything SearchOps knows about one prospect: profile, articles, contacts with trust levels, score, domain metrics.", input_schema: obj({ author_id: str("The author's id") }, ["author_id"]) }),
  ...
  // ── act (reversible) ──
  T({ name: "create_backlink_campaign", description: "Create (or return the existing) backlink campaign for a target page on northwind.example, with its workflow and paid-collab template. Reversible: creates rows, sends nothing. An optional custom name makes the name the campaign's identity, so several campaigns can target the SAME page with separate prospect lists — offer this when someone wants their own campaign or a page already has one.", input_schema: obj({ target: str("Target path or URL, e.g. /ai-image-generator"), name: str("Optional campaign name, e.g. 'Backlink Campaign - Arham'. Same name returns the same campaign; omit to reuse the page's default campaign") }, ["target"]) }),
  ...
  T({ name: "run_url_sweep", description: "Crawl every page on northwind.example and find EVERY place we still link to a retired URL — old dashboard paths, dead subdomains, anything being decommissioned. Answers 'where are all the references so we can remove them in one go', which the broken-link audit cannot: a retired URL usually still returns 200 or redirects, so that audit sees it as healthy and discards it. Reads pages only; changes nothing on the site. Runs in the background over ~1,500 pages and takes several minutes — start it, say so, and read url_sweep_report afterwards rather than waiting. With no patterns it looks for the known retired set (/dashboard, ideate/shorts/trust subdomains, plus any other northwind.example subdomain).", input_schema: obj({ patterns: { type: "array", items: { type: "string" }, description: "Optional. What to hunt for: a path like '/dashboard' or a host like 'old.northwind.example'. Omit for the standing retired-URL set." } }) }),
...
The summary must state the honest scope ("34 emails from waleed@'s Gmail", not "the queue").`,
  ...
  T({ name: "competitor_link_intersect", description: "The strongest prospect source: domains linking to TWO OR MORE competitors but not to northwind.example — proven link-placers in our niche. Fetches each profile through the 14-day cache (repeat runs are free; cache misses bill ~100 units per competitor + up to 500 for our own profile, say the worst case first). READ ONLY — present with show_picker and WAIT, same rules as competitor_backlinks.", input_schema: obj({ competitors: { type: "array", items: { type: "string" }, description: "2-4 competitor domains" }, min_dr: num("Minimum linking-domain DR, default 30"), max_dr: num("Maximum linking-domain DR, default 85"), per_competitor_limit: num("Rows per competitor profile, default 100, max 200"), campaign_id: str("Optional backlink_campaigns.id — marks rows whose domain is already a prospect there") }, ["competitors"]) }),
  ...
  try {
    switch (name) {
      ...
      case "imagine_videos": {
        ...
        return ok({
          channel: Y.NORTHWIND_CHANNEL_URL,
          matched: hits.map((v) => ({
      ...
      case "house_style": {
        ...
        const BY_SURFACE: Record<string, string[]> = {
          blog: ["northwinds-house", "arooj-ishtiaq-blog-writing-tone"],
          landing: ["northwinds-landing", "northwinds-feature-page"],
          feature_page: ["northwinds-feature-page", "northwinds-landing"],
          fnb: ["misher-fnb"],
        };
